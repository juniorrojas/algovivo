...
#define loss_backward() { \
  ...
  backward_euler_loss_grad(space_dim, g, h, num_vertices, pos0, vel0, vertex_mass, num_muscles, muscles, k, a, l0, num_triangles, triangles, rsi, mu, lambda, k_collision, k_friction, pos, pos_grad); \
...
#define optim_step() { \
  ...
  const auto loss0 = backward_euler_loss(space_dim, g, h, num_vertices, pos0, vel0, vertex_mass, num_muscles, muscles, k, a, l0, num_triangles, triangles, rsi, mu, lambda, k_collision, k_friction, pos); \
  for (int i = 0; i < max_line_search_iters; i++) { \
    add_scaled(num_vertices * space_dim, pos, pos_grad, -step_size, pos_tmp); \
    const auto loss1 = backward_euler_loss(space_dim, g, h, num_vertices, pos0, vel0, vertex_mass, num_muscles, muscles, k, a, l0, num_triangles, triangles, rsi, mu, lambda, k_collision, k_friction, pos_tmp); \
    if (loss1 < loss0) { \
      break; \
    } else { \
